...
class DataSequence : public Sequence<UINT, RANGE> {
  ...
  public:
    ...
    DataSequence(const unsigned num_strands, std::string &filename)
        ...
        while (std::getline(fhandle, line)) {
            if (line[0] != '=') {
                ...
                data.push_back(number %
                               RANGE); // only care corrsponding number of bits
                sequence_counter++;
            } else {
        ...
        fhandle.close();
        std::uniform_int_distribution<UINT> sampling_distribution(
            0, sequence_counter - 1);
    ...
    UINT operator()() {
        if (with_file_input) {
            ...
            return std::distance(partition.begin(),
                                 std::lower_bound(partition.begin(),
                                                  partition.end(), dist(rng)));
        }
    }
};
